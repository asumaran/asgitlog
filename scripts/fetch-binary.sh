#!/bin/sh
# fetch-binary.sh — the plugin's [[build]] command: provision ./asgitlog without
...
# install.
#
# Set ASGITLOG_BUILD_FROM_SOURCE=1 to skip the download and always compile
# locally (for users who prefer not to run prebuilt binaries):
#   ASGITLOG_BUILD_FROM_SOURCE=1 herdr plugin install asumaran/asgitlog
set -eu
...
URL="https://github.com/asumaran/asgitlog/releases/download/v${VERSION}/asgitlog-${OS}-${ARCH}"

if [ "${ASGITLOG_BUILD_FROM_SOURCE:-0}" = "1" ]; then
  echo "fetch-binary: ASGITLOG_BUILD_FROM_SOURCE=1, skipping release download"
elif [ -n "$OS" ] && [ -n "$ARCH" ] && command -v curl >/dev/null 2>&1; then
  tmp="$(mktemp)"
  if curl -fsSL --retry 2 -o "$tmp" "$URL"; then
    chmod +x "$tmp"
    mv "$tmp" asgitlog
    echo "fetch-binary: installed asgitlog-${OS}-${ARCH} from release v${VERSION}"
    exit 0
  fi
  rm -f "$tmp"
...
if command -v go >/dev/null 2>&1; then
  go build -ldflags "-X main.version=v${VERSION}-source" -o asgitlog .
  echo "fetch-binary: built asgitlog from source (v${VERSION}-source)"
  exit 0
fi
